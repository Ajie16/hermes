...
## 2026-04-20 07:44 - GitHub Search API topic filter 返回 422

**错误现象**: `GET /search/repositories?q=topic:artificial-intelligence+...` 返回 HTTP 422 Unprocessable Entity

**原因分析**: 
GitHub Search API 的 topic 过滤语法可能不同，或者 topic 过滤有特殊要求

**解决方案**: 
改用纯关键字搜索：`q=AI+OR+LLM+OR+agent+created:>2026-04-01`
用 `+` 代替 `OR`，让 GitHub 解析为搜索关键字

**预防措施**: 
GitHub Search API 优先使用关键字组合而非 topic 过滤
遇到 422 时先简化查询条件
