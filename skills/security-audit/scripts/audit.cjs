...
const CLAWDBOT_DIR = process.env.CLAWDBOT_DIR || 
                      (process.argv.includes('--hermes') ? require('os').homedir() + '/.hermes' : '/root/clawd');
const CONFIG_DIR = process.env.CONFIG_DIR || 
                    path.join(CLAWDBOT_DIR, 'skills/.env');
const DOCKER_DIR = process.env.DOCKER_DIR || CLAWDBOT_DIR;
